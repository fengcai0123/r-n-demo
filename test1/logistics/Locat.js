...
//var Component = React.createClass({
    ...
    getInitialState: function(){
        return {
            ...
            modalVisible: false,
            ProvinceIndex:0,
            provinceSelected:data[0]['name'],
            citySelected:data[0]['city'][0]['name'],
            CityIndex:0,
            countySelected:data[0]['city'][0]['area'][0]
        };
    },
    provinceChoose: function(option,i){
        var self = this;
        data.map(function(item,i){
            if(item['name'] == option){
                self.setState({
                    ...
                    CityIndex:0,
                    citySelected:data[i]['city'][0]['name'],
                    countySelected:data[i]['city'][0]['area'][0]
                });
            }
        })
    },
    cityChoose: function(option){
        var self = this;
        var ProvinceIndex = this.state.ProvinceIndex;
        data[this.state.ProvinceIndex]['city'].map(function(item,i){
            if(item['name'] == option){
                self.setState({
                    CityIndex:i,
                    citySelected:option,
                    countySelected:data[ProvinceIndex]['city'][i]['area'][0]
                });
            }
        })
    },
    countyChoose: function(){

    },
    render: function() {
        var province = this.state.provinceSelected;
        var CityIndex = this.state.CityIndex;
        var city = data[this.state.ProvinceIndex]['city']||data[0]['city'];
        var county = data[this.state.ProvinceIndex]['city'][CityIndex]['area']||data[0]['city'][0]['area'];
        return (
            <Modal
                animated={true}
                transparent={true}
                    ...
                    <View style={styles.modalContainer}>
                        <View style={styles.buttonView}>
                            <TouchableOpacity onPress={() => {
                                ...
                                <Text style={{color:this.state.color}}>取消</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => {
                              console.log(this.state.citySelected);
                                    if(this.props.onSubmit) this.props.onSubmit(this.state.provinceSelected,this.state.citySelected,this.state.countySelected);
                                    this.setState({modalVisible: false});
                                }}>
                                <Text style={{color:this.state.color}}>确认</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={styles.mainBox}>
                            <PickerIOS
                                ...
                                onValueChange={this.provinceChoose}
                                >
                                {data.map((option, i) => {
                            ...
                            <PickerIOS
                                ref={'picker'}
                                style={styles.bottomPicker}
                                selectedValue={this.state.countySelected}
                                onValueChange={(option) => this.setState({countySelected: option})}
                                >
                                {county.map((option, i) => {
                                    var label = option;
                                    return (
